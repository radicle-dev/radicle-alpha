...
test_source_files = do
    tests <- testFiles ["rad/monadic/issues.rad", "rad/monadic/names.rad"]
    pure $ testGroup "Radicle source file tests" tests
  where
    testFiles :: [FilePath] -> IO [TestTree]
    testFiles files = do
        ...
        let loads = T.unlines [ "(load! \"" <> toS file <> "\")" | file <- files ]
        ...
        pure $ testCase "doesn't throw" doesntThrow : makeTests (reverse out)

    -- | Takes an output generated by the @:test@ macro and turns it into
    -- a 'TestTree' that passes if the macro test passes.
    makeTests :: [Text] -> [TestTree]
    makeTests lines = do
        line <- lines
        if
            | "ok" `T.isPrefixOf` line ->
                pure $ testCase (drop 2 $ toS line) $ pure ()
            | "not ok" `T.isPrefixOf` line ->
                pure $ testCase (drop 6 $ toS line) $ assertFailure "failed"
            | otherwise -> mzero
